# DAY019 — HospitalAccessControl  
## Publikacja aplikacji na APP01 / IIS

## Cel dnia

Celem DAY019 jest: **publikacja Release, konfiguracja IIS, AppPool, Windows Authentication i test z CLIENT01**.
...
## Efekt końcowy DAY019

Po zakończeniu tego dnia powinieneś mieć:

```text
działający zakres: publikacja Release, konfiguracja IIS, AppPool, Windows Authentication i test z CLIENT01
komendy kontrolne
testy ręczne
...
# Krok 3 — wykonanie zakresu DAY019

Zakres techniczny:

```text
publikacja Release, konfiguracja IIS, AppPool, Windows Authentication i test z CLIENT01
```
...
```powershell
New-Item -ItemType Directory -Force .\docs\day019
New-Item -ItemType File -Force .\docs\day019\README.md
```
...
git status
git add .
git commit -m "DAY019 Publikacja aplikacji na APP01 / IIS"
```

---

# Kontrola końcowa DAY019

```text
[ ] dotnet build działa
[ ] aplikacja uruchamia się
[ ] zakres dnia został wykonany: publikacja Release, konfiguracja IIS, AppPool, Windows Authentication i test z CLIENT01
...
W ramach etapu DAY019 rozbudowano projekt HospitalAccessControl o obszar: Publikacja aplikacji na APP01 / IIS. Zakres ten uzupełnia demonstrację kontroli dostępu, audytu oraz bezpieczeństwa danych medycznych w środowisku .NET i SQL Server.
